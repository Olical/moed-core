...
test('can create and fetch a buffers contents', function (t) {
	...
	t.strictEqual(buffer.lines.join(lineBreakCharacter), content, 'content was retrieved (lines)');
	t.strictEqual(this.b.getContent(buffer.identifier), content, 'content was retrieved (full)');
}.bind(setup()));
